...
class TestFileHandler:
    ...
    def test_process_csv(self, handler, mock_csv_content, mocker):
        # Mock S3 client
        mocker.patch.object(
            handler.s3_client, "head_object", return_value={"ContentLength": 100}
        )
        mocker.patch.object(
            handler.s3_client,
            "get_object",
        ...
        result = handler.process("s3://test-bucket/test.csv", ["name", "email"])
    ...
    def test_process_json(self, handler, mock_json_content, mocker):
        ...
        result = handler.process("s3://test-bucket/test.json", ["name", "email"])
    ...
    def test_file_size_limit(self, handler, mocker):
        ...
        mocker.patch.object(
            handler.s3_client, "head_object", return_value={"ContentLength": 2_000_000}
        )

        with pytest.raises(ValueError, match="File size .* exceeds limit"):
            handler.process("s3://test-bucket/test.csv", ["name"])
    ...
    def test_file_not_found(self, handler, mocker):
        ...
        with pytest.raises(FileNotFoundError):
            handler.process("s3://test-bucket/missing.csv", ["name"])

    def test_empty_file(self, handler, mocker):
        # Mock empty file
        mocker.patch.object(
            handler.s3_client, "head_object", return_value={"ContentLength": 0}
        )
        mocker.patch.object(
            handler.s3_client,
            "get_object",
        ...
        with pytest.raises(ValueError, match="File is empty"):
            handler.process("s3://test-bucket/empty.csv", ["name"])
    ...
    def test_missing_fields(self, handler, mock_csv_content, mocker):
        ...
        with pytest.raises(ValueError, match="Fields not found"):
            handler.process("s3://test-bucket/test.csv", ["non_existent_field"])
